...
method_path = Path("docs/hcc_methodology_v2.md")
builder_path = Path("scripts/build_hcc_manuscript_v1.py")

method_text = """# High-Confidence Computing Methodology v2
...
if builder_path.exists():
    ...
    b = b.replace('ROOT / "docs" / "hcc_methodology_v1.md"', 'ROOT / "docs" / "hcc_methodology_v2.md"')
    lines = b.splitlines()
    marker = '    text = text.replace("# High-Confidence Computing Methodology v2", "")'
    if marker not in lines:
        for idx, line in enumerate(lines):
            if 'text = text.replace("# High-Confidence Computing Methodology v1", "")' in line:
                lines.insert(idx + 1, marker)
                break
    builder_path.write_text("\n".join(lines) + "\n")

print(f"Saved: {method_path}")
print("Updated manuscript builder to use Methodology v2.")
